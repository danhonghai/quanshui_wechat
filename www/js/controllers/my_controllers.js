angular.module('my.controllers', [])
    ...
    //登入注册
    .controller('LoginCtrl', ['$scope', '$rootScope', 'Services', '$ionicPopup', '$ionicLoading', '$state', '$stateParams', '$interval', '$ionicModal', function(
        $scope, $rootScope, Services, $ionicPopup, $ionicLoading, $state, $stateParams, $interval, $ionicModal) {
        console.log($stateParams.inviteCode)
        $scope.data = {};
        $scope.passdata = {};
        $scope.regdata = {};
        $scope.forpassdata = {};
        ...
        $scope.getphoneCodefun = function(){
            if ($scope.regdata.userName) {
                ...
                Services.getData("noauth/sendMessage", 1, senddata, function(data){
                    ...
                    // Services.ionicLoading(2000, "验证码发送成功，请注意查收。");
                })
            }else{
                Services.ionicLoading(2000, "请输入手机号");
            }
            
        }
            //注册
        if ($stateParams.inviteCode) {
            $scope.regdata.inviteCode = $stateParams.inviteCode
        }
    ...
    //充值
    .controller('RechargeCtrl', ['$scope', '$state', '$timeout', '$ionicModal', 'Services', '$ionicLoading', function($scope, $state, $timeout, $ionicModal, Services, $ionicLoading) {
        $scope.data = {};
    ...
    //提现
    .controller('WithdrawalCtrl', ['$scope', '$state', '$timeout', '$ionicModal', 'Services', '$ionicLoading', function($scope, $state, $timeout, $ionicModal, Services, $ionicLoading) {
        $scope.data = {};
        $scope.userinfo = angular.fromJson(sessionStorage.userinfo);
        // //console.log($scope.userinfo);
        $scope.withdrawalbtn = function(){
            if ($scope.data.amount) {
                ...
                Services.ionicLoading(2000, '请输入提现金额');
            }
        }
        $scope.clearmoney = function(){
            $scope.data.amount = "";
    ...
    .controller('BorrowmanageCtrl', ['$scope', 'Services', '$ionicPopup', '$state', function($scope, Services, $ionicPopup, $state) {
        ...
        $scope.repaylink = function(borrowRepaymentId,lateid){
            console.log(lateid)
            $state.go("repayments",{"repaymentId":borrowRepaymentId,"lateid":lateid});
        }
    }])
    .controller('RepaymentsCtrl', ['$scope', 'Services', '$ionicPopup', '$state', '$stateParams', '$ionicLoading', function($scope, Services, $ionicPopup, $state, $stateParams, $ionicLoading) {
        Services.ionicLoading();
        $scope.data = {};
        var objdata = {
        ...
        $scope.repayfun = function(){
            ...
            if ($stateParams.lateid == 0) {
                ...
                Services.getData("overdueRepayment", 1, objdata, function(data){
                    Services.console(data);
                    $ionicLoading.hide()
                    $scope.optionsPopup = $ionicPopup.show({
                        template: "还款成功",
    ...
    //红包加息券
    .controller('MyticketCtrl', ['$scope', 'Services', '$ionicScrollDelegate', '$ionicPopup', '$state', '$stateParams', '$ionicLoading', function($scope, Services, $ionicScrollDelegate, $ionicPopup, $state, $stateParams, $ionicLoading) {
        $scope.pageNumber = 0; //分页的第几页
        $scope.pageSize = 10; //分页一页显示几条
    ...
    //积分签到
    .controller('MypointsCtrl', ['$scope', '$timeout', '$stateParams', 'Services', '$ionicLoading', '$ionicModal', function($scope, $timeout, $stateParams, Services, $ionicLoading, $ionicModal) {
        Services.ionicLoading();
        ...
        $scope.getqdlist = function(year, month) {
                ...
                Services.getData("signLog", 1, pdparameterObj, function(data){
                    ...
                    if (data.code == "0000") {
                        ...
                        $scope.rules = data.data.list.concat(data.data.list1)
    ...
    .controller('SettingCtrl', ['$scope', '$timeout', '$stateParams', '$state', 'Services', '$ionicPopup', function($scope, $timeout,
        ...
        Services.getDataget("showAccount", "", function(data){
            Services.console(data);
            $scope.settingdata = data.data.userInfo;
            $scope.userAccount = data.data.userAccount;
            $scope.userCredit = data.data.userCredit;
        })
        //退出登入
        $scope.exitfun = function() {
            Services.getDataget("logout", "", function(data){
                Services.console(data);
                sessionStorage.token = "";
                sessionStorage.__tempCache = "";
                sessionStorage.userinfo = "";
                $scope.optionsPopup = $ionicPopup.show({
                    template: "<h4>温馨提示</h4>" + "<p>账号退出成功</p>",
                    title: "温馨提示",
                    scope: $scope,
                    buttons: [{
                        text: "重新登录",
    ...
    .controller('AutoinvestCtrl', ['$scope', '$timeout', '$stateParams', '$state', 'Services', '$ionicPopup', '$ionicLoading', function($scope, $timeout,
        $stateParams, $state, Services, $ionicPopup, $ionicLoading) {
        Services.ionicLoading();
        $scope.data = {
            tenderMoneyType:3,
            status:2,
        ...
        $scope.autoinvestfun = function(){
            ...
            if (!$scope.data.bestChoose) {
                Services.ionicLoading(2000, "请选择优惠券")
                return false
            }
            if (!$scope.data.bestStrategy) {
                Services.ionicLoading(2000, "请选择优惠策略")
                return false
            }
        ...
        function($scope, $timeout, $rootScope, Services, $ionicPopup, $ionicLoading, $state,
            ...
            $scope.getemailCodefun = function() {
                ...
                if ($scope.data.email) {
                    ...
                    Services.ionicLoading(2000, "请输入邮箱")
                }
            }
            //发送手机验证码
            $scope.getphoneCodefun = function() {
    ...
    .controller('InvitefriendsCtrl', ['$scope', '$timeout', 'Services', function($scope, $timeout, Services) {
        ...
        $scope.invitedbtn = function() {
            Services.ionicLoading(2000, "复制成功,您可以使用粘贴操作了！");
        }
    }])
    //银行卡管理
